...
export const DEFAULT_PROMPTS = {
  ...
  `,
/**
   * TESTDATA Only Prompt
   */
  TESTDATA_ONLY: `
     Instructions:
    - Generate ONLY a testdata file.
    - Use Examples table for the testdata format.
    - Generate maximum 20 data sets with atleast 5 negative cases and 2 edge cases.
    - Make sure columns are relevant to the provided DOM.
    - Use different regions realistic dataset (names, addresses, pin codes, mobile numbers).
    ...
    Persona:
    - Audience: Manual testers who only need testdata files.

    Output Format:
    - Only valid testdata in a \`\`\`gherkin\`\`\` block.
   ...
   */
  PLAYWRIGHT_TYPESCRIPT_PAGE_ONLY:`Instructions:
    - Generate ONLY a Playwright Typescript Page Object Class (no test code).
    - Add Doc for methods & class.
    - Use Playwright v1.56+ version and best practices for TypeScript.
    - Use meaningful method names.
    - Do NOT include explanations or test code.
